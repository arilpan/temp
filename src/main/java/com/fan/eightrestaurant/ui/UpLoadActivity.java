...
public class UpLoadActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private String user_id, nickname, imagePath, token,
            secretkey, realname, birthday, email;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.activity_up_load_name:
                createUserDialog = new CreateUserDialogUtils(this, R.style.loading_dialog, onClickListener, "请输入昵称");
                createUserDialog.show();
                break;
            case R.id.activity_up_load_realname:
                createUserDialog = new CreateUserDialogUtils(this, R.style.loading_dialog, onClickListener, "请输入真实姓名");
                createUserDialog.show();
                break;
            case R.id.activity_up_load_birthday:
                createUserDialog = new CreateUserDialogUtils(this, R.style.loading_dialog, onClickListener, "请输入生日");
                createUserDialog.show();
                break;
            case R.id.activity_up_load_email:
                createUserDialog = new CreateUserDialogUtils(this, R.style.loading_dialog, onClickListener, "请输入邮箱");
                createUserDialog.show();
                break;
        }
    }
            ...
            switch (v.getId()) {
                ...
                case R.id.create_user_dialog_savebrithday:   //用户修改生日
                    ...
                    if (year % 4 == 0 || (year % 4 == 0 && year % 100 == 0)) {
                        regEx = "(^[1-2][0-9][0-9][0-9][-](([0][1,3,5,7,8])|([1][0,2]))[-](([0][0-9])|([1-2][0-9])|([3][0-1]))$)|(^[1-2][0-9][0-9][0-9][-](([0][4,6,9])|([1][1]))[-](([0][0-9])|([1-2][0-9])|([3][0]))$)|(^[1-2][0-9][0-9][0-9][-](([0][2]))[-](([0][1-9])|([1-2][0-9]))$)";
                    } else {
                        regEx = "(^[1-2][0-9][0-9][0-9][-](([0][1,3,5,7,8])|([1][0,2]))[-](([0][0-9])|([1-2][0-9])|([3][0-1]))$)|(^[1-2][0-9][0-9][0-9][-](([0][4,6,9])|([1][1]))[-](([0][0-9])|([1-2][0-9])|([3][0]))$)|(^[1-2][0-9][0-9][0-9][-](([0][2]))[-](([0][1-9])|([1][0-9])|([2][0-8]))$)";
                    }
                    if (brithday.equals("")) {
                ...
                case R.id.create_user_dialog_saveemail:  //用户修改邮箱
                    ...
                    regEm = "^([a-z0-9A-Z]+[-|\\.]?)+[a-z0-9A-Z]@([a-z0-9A-Z]+(-[a-z0-9A-Z]+)?\\.)+[a-zA-Z]{2,}$";
                    if (email.equals("")) {
    ...
    private void uploadPic(Bitmap photo) {
        final String imageUrl = UpLoadUtils.savePhoto(photo, Environment
                    ...
                    .execute(new StringCallback() {
                        ...
                        public void onResponse(String response) {
                            try {
                                ...
                                imagePath = APIAddr.IP_PORT + jb.getString("value");
                                updateUserInfo(nickname, realname, birthday, email, imagePath);
