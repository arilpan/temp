    ...
    public void onClick(View v)
    {
        switch (v.getId())
        {
            case R.id.retrieve_button:
                if (userPwd.getText().toString().trim().length() < 6 || userPwd.getText()
                        .toString().trim().length() > 16)
                {
                    Toast.makeText(RetrievePwdActivity.this, "密码为6-16位数字、字母组成", Toast
                            .LENGTH_SHORT).show();
                } else
                {
                    OkHttpUtils
                            ...
                            .execute(new StringCallback()
                            {
                                ...
                                public void onResponse(String response)
                                {
                                    try
                                    {
                                        ...
                                        {
                                            Toast.makeText(RetrievePwdActivity.this, "服务器出问题了",
                                                    Toast.LENGTH_SHORT).show();
                                        } else if (message.equals("ok"))
                                        {
                                            Toast.makeText(RetrievePwdActivity.this, "修改成功",
                                                    Toast.LENGTH_SHORT).show();
                            ...
                            .execute(new StringCallback()
                            {
                                ...
                                public void onResponse(String response)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        {
                                            Toast.makeText(RetrievePwdActivity.this, "信息发送失败",
                                                    Toast.LENGTH_SHORT).show();
                                        } else if (message.equals("repetition"))
                                        {
                                            Toast.makeText(RetrievePwdActivity.this, "手机号码已被注册",
                                                    Toast.LENGTH_SHORT).show();
                                        } else if (message.equals("error"))
                                        {
                                            Toast.makeText(RetrievePwdActivity.this, "手机号码格式错误",
                                                    Toast.LENGTH_SHORT).show();
                                        } else if (message.equals("ok"))
                                        {
                                            Toast.makeText(RetrievePwdActivity.this, "信息发送成功",
                                                    Toast.LENGTH_SHORT).show();
                                        }
                                    } catch (JSONException e)
                                    {
